...
// Filter headings based on active tab for TOC
document.addEventListener('DOMContentLoaded', function() {
  // Check if the current page uses the tabs layout
  const isTabLayout = document.querySelector('meta[name="page-layout"]')?.getAttribute('content') === 'docs-component-tabs.njk';
  
  // Only apply the TOC filtering if we're using the tabs layout
  if (isTabLayout) {
    // Function to update which headings are visible to the TOC
    function updateTOCVisibility() {
      // First, mark all headings to be ignored by TOC
